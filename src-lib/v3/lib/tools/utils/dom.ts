...
export async function findCodeFromDOM(): Promise<string | void> {
  try {
    ...
    if (customSources.length === 0) {
      const code = await findCodeFromBody();
      if (code) customSources.push(code);
    }
...
async function findCodeFromBody(): Promise<string> {
  const bodyHTML = document.body.innerText;
  const cleaned = bodyHTML.replace(/<[^>]*>[\s\S]*?<\/[^>]*>/g, "");
  return cleaned;
}

export async function findCSSFromBody(): Promise<string> {
  const cssTags = document.body.querySelectorAll("style");
  let cssCode = "";
  if (cssTags.length > 0) {
    for (let i = 0; i < cssTags.length; i++) {
      cssCode += cssTags.item(i).textContent;
    }
  }
  return cssCode;
}
